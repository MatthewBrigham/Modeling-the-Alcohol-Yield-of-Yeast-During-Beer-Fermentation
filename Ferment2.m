function Ferment2(N, sub_mass, V)
...
T = 24 + 273.15;                     %convert input temperature of Celsius to Kelvin
N = N/V;         % g/L Number of inoculated cells-Computes the approximate number of active yeast cells inoculated
S0 = 80; %sub_mass*453.592/V;      %Initial Substrate Conc. Converts lbs to grams/L of initial substrate
...
C_s(1) = S0; %S0/V;                      %initial concentration of sugar substratein g/L
...
%keyboard
for n = 2:length(t)-2     
    if X_act(n-1) < 0.8*N                       %lag phase population modeling
        X_act(n) = X_act(n-1) + deltat.*(mu_lag*X_lag(n-1));
        X_lag(n) = mu_lag.*(N-X_act(n-1));
    end
    
    if X_act(n-1) >= 0.8*N                     %Growth phase modeling (fermentation begins)
    X_act(n) = X_act(n-1) + deltat.*(X_act(n-1).*(mu_x(n-1)-mu_DT) + X_lag(n-1).*mu_lag);
    
    C_s(n) = C_s(n-1) - deltat.*(mu_s(n-1).*X_act(n-1));
    C_e(n) = C_e(n-1) + deltat.*(f(n-1).*mu_e(n-1).*X_act(n-1));
    
    mu_x(n) = mu_x0.*C_s(n-1)./(k_x+C_s(n-1));        
    mu_s(n) = mu_s0.*C_s(n-1)./(k_s+C_s(n-1));
    mu_e(n) = mu_e0.*C_s(n-1)./(k_e+C_s(n-1));
    f(n) = 1-C_e(n-1)./(0.5*S0);                      
    end
...
title(sprintf('Cell Biomass for T = %d C', T-273.15));
xlabel('t [hrs]');
ylabel('Active Cells [g/L]');

figure(2);
plot(t(1:n-2), C_s(1:n-2));
title(sprintf('Sugar Concentration for T = %d C', T-273.15));
xlabel('t [hrs]');
ylabel('C_s [g/L]');

figure(3);
plot(t(1:n-2), C_e(1:n-2));
title(sprintf('Ethanol Concentration for T = %d C', T-273.15));
xlabel('t [hrs]');
ylabel('C_e [g/L]');
